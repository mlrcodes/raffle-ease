package com.raffleease.raffles_service.Feign.Decoders;
...
public class CustomErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String methodKey, Response response) {
        ...
        return switch (status) {
            ...
            case CONFLICT -> {
                assert apiError != null;
                yield new ConflictException(apiError.getMessage());
            }
            case BAD_REQUEST -> {
                assert apiError != null;
                yield new IllegalArgumentException(apiError.getMessage());
            }
